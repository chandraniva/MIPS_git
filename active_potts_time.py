...
@jit
def update_mov(s,beta):
    for r in range(sqL):
        i= int(np.random.rand()*sqL)
        if s[i]>0:  #movement
                j = nbr[i,np.random.randint(5)]
                if s[j] == 0:    
                    eng_i = H_mov(s,i)
                    s[j] = s[i]
                    s[i] = 0
...
ax2.plot(np.arange(time),op/ens,'.-',label='beta='+str(beta))
ax2.set_ylim(0.05,1)
ax2.legend()
plt.show()


# np.savetxt("ic0_AP_time_L="+str(L)+"_beta="+str(beta)+"_ens="+str(ens)+"_rho="+
#         str(rho)+"_J="+str(J)+".txt",op)
